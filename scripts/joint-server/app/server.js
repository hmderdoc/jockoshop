...
// killerdraw patch 1/2: was `require("discord.js")`. Only used for the optional --discord
// webhook; see ./discord_stub.js. Keeps discord.js out of this copy's dependencies.
const {WebhookClient} = require("./discord_stub");
let hourly_saver;
...
// killerdraw patch 2/2: `server` (the module-level http server every joint shares) added to the
// exports. start_joint() calls server.listen(port) without awaiting "listening", so a caller that
// asks for port 0 has no way to learn the port it actually got. The rig listens itself first,
// waits for "listening", then calls start_joint (which sees server.address() and skips its own
// listen). Nothing else changed.
module.exports = {close, start_joint, end_joint, has_joint, server};
