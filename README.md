...
- **Canvas width** is a set of the widths art is drawn at — 40 (C64), 77 (BBS ad), 80, 132, 160 — applied in one step, with Custom… behind a dialog that changes nothing until Apply. **Height** commits when you stop typing rather than resizing on every digit, and **grow** (on by default) gives the canvas another row when the typewriter caret runs off the bottom instead of stopping.
