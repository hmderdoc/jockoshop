...
- **XBIN (`.xb`) export embeds the font** the art is drawn in, so the file opens in that font whatever the viewer defaults to — which is the reason to pick XBIN over `.ans`. Reading one back draws it in its own font too.
